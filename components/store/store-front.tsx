...
export function StoreFront({ store, categories }: { store: any, categories: any[] }) {
  ...
  const handleProductClick = (product: Product) => { setSelectedProduct(product); setTempObservation(""); setTempRemovedIngredients([]); setTempSelectedAddons([]); setItemQuantity(1); }
  
  const calculateItemTotal = (product: Product, selectedAddonIds: string[]) => {
      const addonsTotal = product.addons?.filter(a => selectedAddonIds.includes(a.id)).reduce((sum, a) => sum + a.price, 0) || 0
      ...
      <div className="relative w-full bg-slate-900 overflow-hidden">
        ...
        <div className="relative h-[40vh] md:h-[350px] w-full">
            {banners.length > 0 ? banners.map((img: string, index: number) => (
                ...
                <div className="w-full h-full flex items-center justify-center bg-slate-800"><span className="text-white/30 text-sm">Sem Imagens</span></div>
            )}
        </div>
      ...
      <div className="bg-white border-b border-slate-100 shadow-sm relative z-20">
          <div className="max-w-7xl mx-auto px-4 md:px-8 py-3 flex justify-between gap-3">
              <button onClick={() => setIsInfoOpen(true)} className="flex items-center gap-2 group text-left hover:bg-slate-50 p-1.5 -ml-1.5 rounded-lg transition-colors">
                  <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-primary group-hover:bg-primary/10 transition-colors"><MapPin className="w-4 h-4" /></div>
                  <div className="flex flex-col"><span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Localização</span><div className="flex items-center gap-1"><span className="text-sm font-bold text-slate-900 line-clamp-1 max-w-[250px] md:max-w-md">{storeAddress}</span><ChevronRight className="w-3 h-3 text-slate-400" /></div></div>
              </button>
              <div className="flex items-center gap-3">
                  <div className={cn("px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5", isOpenNow ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700")}>
                      <Clock className="w-3.5 h-3.5" />{isOpenNow ? "Loja Aberta" : "Loja Fechada"}
                  </div>
              </div>
          </div>
      </div>
        ...
        <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-6 py-2">
                <div className="flex gap-3 items-start bg-slate-50 p-3 rounded-lg border">
                    ...
                    <div><h4 className="font-bold text-sm text-slate-900">Endereço</h4><p className="text-sm text-slate-600 mt-1">{storeAddress}{store.zip_code && <span className="block text-xs text-slate-400 mt-0.5">CEP: {store.zip_code}</span>}</p></div>
                </div>
                <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">{activeDays.length > 0 ? activeDays.map((day: any, index: number) => (<div key={index} className="bg-slate-50 p-2 rounded text-xs border flex flex-col justify-center"><span className="font-bold text-slate-700">{day.day}</span><span className="text-slate-500">{day.open} - {day.close}</span></div>)) : <p className="text-sm text-slate-500 italic">Fechado.</p>}</div>
                </div>
            </div>
        </DialogContent>
      </Dialog>
